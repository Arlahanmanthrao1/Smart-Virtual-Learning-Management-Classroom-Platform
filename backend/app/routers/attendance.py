...
def list_sessions(course_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return (
        db.query(ClassSession)
        .filter(ClassSession.course_id == course_id, ClassSession.ended_at.is_(None))
        .order_by(ClassSession.scheduled_at.desc())
        .all()
    )
...
def end_session(
    ...
    for record in open_records:
        ...
        if student and course:
            sync_attendance_to_erp(student.email, course.code, record.duration_minutes, record.present)
    return session
